# CI/CD - Drone, Gogs and Minikube w/local Registry on OS X
	

Docker-Compose is used to instantiate the environment and run locally. The GOGS container uses Sqlite3 and provides persistent storage for your repos. Minikube will use the VM Driver with VirtualBox. This branch version is leveraging a local registry.
...
- We are going to use the `k8-startup.sh` script. This script will start up minikube. However, it will delete any instance of minikube that you have. __Edit this file if you don't want to delete anything!__ 
	- The commands you might want to comment out are:
	> * minikube delete
	> * sudo rm -rf ~/.minikube
	> * sudo rm -rf ~/.kub
...
- Enter `./drone-gogs-startup.sh` to start up Drone and Gogs. When the script is done, run `docker ps` and you should see containers: dc-gogs and dc-drone-server. Also, run: `docker network ls` and make sure you see a network named `cicd-demo_default`. 
...
- Open a browser to `http://localhost:3000` (port 3000 is defined in the yaml file).
	...
	* Create an admin user. This will be your account throughout this application.
	* Hit submit. Note that the browser might not refresh. Just open another browser to `http://localhost:3000`  You'll probably already be logged in. Create a repo called: `cicd-app`. You must name this exactly as indicated. 
...
- Go back to your terminal, type: `cd cicd-app` Enter:
	...
	* Go back to your Gogs browser and check your repo to make sure the files are present.

### Section 3: Set up DRONE

- Open another browser to `http://localhost` You should automatically be logged in. If not, enter your user info you created earlier in the Gogs setup. Wait for DRONE to sync. __Do not activate the repo yet__.
